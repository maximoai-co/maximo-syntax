...
import type { CommandId } from "@maximo-syntax/types"
...
import { t } from "../i18n"
import { getAppUrl } from "@maximo-syntax/types" // kilocode_change
...
export function getVisibleProviderOrLog(outputChannel: vscode.OutputChannel): ClineProvider | undefined {
	...
	if (!visibleProvider) {
		outputChannel.appendLine("Cannot find any visible Maximo Syntax instances.")
		return undefined
	}
...
export const openClineInNewTab = async ({ context, outputChannel }: Omit<RegisterCommandOptions, "provider">) => {
	...
	const newPanel = vscode.window.createWebviewPanel(ClineProvider.tabPanelId, "Maximo Syntax", targetCol, {
		enableScripts: true,
