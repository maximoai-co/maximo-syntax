...
import type { CloudUserInfo, AuthState } from "@maximo-syntax/types"
import { CloudService, BridgeOrchestrator } from "@maximo-syntax/cloud"
import { TelemetryService, PostHogTelemetryClient, DebugTelemetryClient } from "@maximo-syntax/telemetry" // kilocode_change: added DebugTelemetryClient
...
export async function activate(context: vscode.ExtensionContext) {
	...
	if (!context.globalState.get("firstInstallCompleted")) {
		outputChannel.appendLine("First installation detected, opening Maximo Syntax sidebar!")
		try {
			await vscode.commands.executeCommand("maximo-syntax.SidebarProvider.focus")

			outputChannel.appendLine("Opening Maximo Syntax walkthrough")
	...
	// kilocode_change start: Fetch Maximo Syntax notifications on startup
	try {
	...
	// kilocode_change start - Maximo Syntax specific registrations
	...
	// kilocode_change end - Maximo Syntax specific registrations

	registerCodeActions(context)
	registerTerminalActions(context)

	// Allows other extensions to activate once Maximo Syntax is ready.
	...
	if (process.env.NODE_ENV === "development") {
		const watchPaths = [
			...
			{ path: path.join(context.extensionPath, "node_modules/@maximo-syntax/cloud"), pattern: "**/*" },
		]
