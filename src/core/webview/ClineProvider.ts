...
import { CloudService, BridgeOrchestrator, getRooCodeApiUrl } from "@maximo-syntax/cloud"
	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	*/
	...
	async handleKiloCodeCallback(token: string) {
		...
		vscode.window.showInformationMessage("Maximo Syntax successfully configured!")
	...
	> {
		...
		const apiProvider: ProviderName = stateValues.apiProvider ? stateValues.apiProvider : "maximo-ai" // maximosyntax_change: fall back to maximo-ai
	...
	async resetState() {
		...
		// Logout from Maximo Syntax provider before resetting (same approach as ProfileView logout)
		const { apiConfiguration, currentApiConfigName = "default" } = await this.getState()
